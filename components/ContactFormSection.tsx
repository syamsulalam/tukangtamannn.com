...
import { Send, MapPin, Phone, Mail, CheckCircle2, AlertCircle, RefreshCw } from "lucide-react";
...
export default function ContactFormSection() {
  ...
  const formatPhoneMask = (val: string) => {
    ...
    // If it starts with +62 or 62
    if (cleaned.startsWith("+62")) {
      const digits = cleaned.slice(3).replace(/\D/g, ""); // only digits after +62
      ...
      // Assume starting with 0 or other digits
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setIsSuccess(true);
        setToastMessage({ 
          text: data.message || "Pesan Anda sukses terkirim ke TukangTamannn.com!", 
          type: "success" 
        });
        setFormState({ name: "", phone: "", service: "", message: "" });
        setTouched({});
        ...
        setToastMessage({ 
          text: data.message || "Gagal mengirim pesan. Silakan hubungi kami via WhatsApp.", 
          type: "error" 
        });
      }
    } catch (err) {
      ...
      setToastMessage({ 
        text: "Koneksi terputus. Silakan hubungi kami langsung via WhatsApp atau tlp di 0821-3147-2864.", 
        type: "error" 
      });
    } finally {
      setIsSubmitting(false);
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl backdrop-blur-md border text-sm font-medium ${
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 relative z-10">
        <div>
          ...
          <p className="text-white/70 font-medium leading-relaxed mb-12 max-w-md">
            Punya ide taman impian? Atau butuh bantuan merawat lanskap yang ada? Tim kami siap mendengarkan dan memberikan solusi terbaik.
          </p>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-white/70 text-sm">082131472864</p>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-white/70 text-sm">halo@tukangtamannn.com</p>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <AnimatePresence mode="wait">
            {isSuccess ? (
              ...
              >
                <div className="w-20 h-20 bg-forest/10 rounded-full flex items-center justify-center text-forest mb-6">
                  <CheckCircle2 size={40} className="text-forest" />
                </div>
                <h3 className="text-2xl font-extrabold text-forest mb-2 tracking-tight">Pesan Terkirim!</h3>
                <p className="text-charcoal/70 font-medium mb-8 max-w-sm">Terima kasih telah menghubungi kami. Tim TukangTamannn.com akan membalas pesan Anda dalam kurun waktu 1x24 jam.</p>
              ...
              >
                ...
                <button 
                  type="submit" 
                  disabled={isSubmitting} 
                  className="w-full bg-forest text-white font-bold py-4 rounded-xl shadow-lg shadow-forest/20 hover:bg-forest/90 transition-all flex items-center justify-center gap-2 group disabled:opacity-75 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
